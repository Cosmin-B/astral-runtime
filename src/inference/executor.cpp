...
AstralErr ASTRAL_CALL apply_grammar_for_slot(void* grammar_ctx, uint32_t* tokens, float* logits, uint32_t count) {
    ...
    return ctx->ops->session_apply_grammar_for_slot(ctx->session_ctx, ctx->slot_id, tokens, logits, count);
}
...
AstralErr ensure_penalty_state(Conversation* conv) {
    ...
    if (repeat_last_n != 0) {
        ...
        if (conv->recent_capacity != repeat_last_n || conv->recent_tokens == nullptr) {
            conv->recent_tokens = static_cast<uint32_t*>(
                conv->allocator.alloc(static_cast<size_t>(repeat_last_n) * sizeof(uint32_t), alignof(uint32_t))
            );
...
inline bool stream_hold_push(Conversation* conv, const concurrency::StreamToken& tok, uint32_t stop_hold) {
    if (conv == nullptr) {
        return false;
    }
...
void executor_thread(ModelExecutor* ex) {
    ...
    while (ex->running.load(std::memory_order_acquire)) {
        ...
        for (uint32_t sid = 0; sid < ex->max_slots; ++sid) {
            Conversation* c = ex->slots[sid].load(std::memory_order_acquire);
            if (c == nullptr) continue;
            if (c->state.load(std::memory_order_acquire) == ConvState::Decoding) {
                any_work = true;
                break;
            }
            if (c->pending_emit_valid != 0) {
                any_work = true;
                break;
            }
        }
        ...
        if (!any_work) {
            ...
            while (ex->running.load(std::memory_order_acquire) && !any_work) {
                ...
                for (uint32_t sid = 0; sid < ex->max_slots; ++sid) {
                    Conversation* c = ex->slots[sid].load(std::memory_order_acquire);
                    if (c == nullptr) continue;
                    if (c->state.load(std::memory_order_acquire) == ConvState::Decoding || c->pending_emit_valid != 0) {
                        any_work = true;
                        break;
                    }
                }
        ...
        for (uint32_t sid = 0; sid < ex->max_slots; ++sid) {
            ...
            const uint32_t cur = conv->exec_refs.fetch_add(1, std::memory_order_acq_rel) + 1u;
            (void)cur;
        ...
        for (uint32_t sid = 0; sid < ex->max_slots; ++sid) {
            ...
            if (conv->grammar_dirty != 0) {
                ...
                } else if (conv->grammar_kind == 2 && ops->session_grammar_set_json_schema_for_slot != nullptr &&
                           conv->grammar_json != nullptr && conv->grammar_json_len > 0) {
                    AstralSpanU8 schema{};
                    schema.data = conv->grammar_json;
                    schema.len = conv->grammar_json_len;
                    gerr = ops->session_grammar_set_json_schema_for_slot(ex->backend_session_ctx, sid, schema);
                }
        ...
        if (eval_err != ASTRAL_OK) {
            ...
            for (uint32_t sid = 0; sid < ex->max_slots; ++sid) {
                Conversation* conv = ex->slots[sid].load(std::memory_order_acquire);
                if (conv == nullptr) continue;
                if (conv->state.load(std::memory_order_acquire) != ConvState::Decoding) continue;
                conv->t_end_ticks = ticks_now();
        ...
        for (uint32_t i = 0; i < batch_count; ++i) {
            ...
            Conversation* conv = sid < ex->max_slots ? ex->slots[sid].load(std::memory_order_acquire) : nullptr;
            if (conv == nullptr) continue;

            conv->exec_refs.fetch_add(1, std::memory_order_acq_rel);
            if (sid >= ex->max_slots || ex->slots[sid].load(std::memory_order_acquire) != conv) {
                conv->exec_refs.fetch_sub(1, std::memory_order_acq_rel);
                continue;
            }
...
void executor_start(ModelExecutor* ex) {
    ...
    ex->batch_tokens = static_cast<AstralBackendBatchToken*>(
        core::runtime_alloc(static_cast<size_t>(ex->max_batch_tokens) * sizeof(AstralBackendBatchToken),
                            alignof(AstralBackendBatchToken))
    );
    ex->batch_output_slots = static_cast<uint32_t*>(
        core::runtime_alloc(static_cast<size_t>(ex->max_batch_tokens) * sizeof(uint32_t), alignof(uint32_t))
    );

    if (ex->indices_buffer == nullptr || ex->batch_tokens == nullptr || ex->batch_output_slots == nullptr) {
        ex->running.store(false, std::memory_order_release);
        return;
    }
    ...
    if (ops->session_create_ex != nullptr) {
        ex->backend_session_ctx = ops->session_create_ex(model->backend_model_ctx, &sd, ex->max_slots, &create_err);
    } else {
...
void executor_stop_and_join(ModelExecutor* ex) {
    ...
    if (ex->indices_buffer) {
        core::runtime_free(ex->indices_buffer, static_cast<size_t>(ex->vocab_size) * sizeof(uint32_t), alignof(uint32_t));
    ...
    if (ex->batch_output_slots) {
        core::runtime_free(ex->batch_output_slots, static_cast<size_t>(ex->max_batch_tokens) * sizeof(uint32_t),
                           alignof(uint32_t));
