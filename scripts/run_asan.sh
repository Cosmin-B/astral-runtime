...
echo "[asan] Build memory gates"
cmake --build "${build_dir}" --target gate_allocations gate_rss_cap -j
...
echo "[asan] Run memory gates"
ctest --test-dir "${build_dir}" -R '^(gate_allocations|gate_rss_cap)$' -V --output-on-failure
