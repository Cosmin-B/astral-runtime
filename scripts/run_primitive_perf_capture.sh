...
write_target_report() {
  ...
  awk '
    ...
    }
    {
      ...
      emit("spsc_transit_p50", p50_for("SPSC transit pcts", $0), "<=25.00", 25.00)
      ...
      emit("mpsc_transit_p50", p50_for("MPSC transit pcts", $0), "<=35.00", 35.00)
      emit("mpsc_ticket_transit_p50", p50_for("MPSC ticket transit pcts", $0), "<=35.00", 35.00)
      ...
      emit("mpsc_ticket_batch_1p", ns_for("MPSC ticket batch 1P/1C", $0), "<=15.00", 15.00)
      emit("mpsc_ticket_batch_2p", ns_for("MPSC ticket batch 2P/1C", $0), "<=60.00", 60.00)
      emit("mpsc_ticket_batch_4p", ns_for("MPSC ticket batch 4P/1C", $0), "<=60.00", 60.00)
      emit("mpsc_ticket_batch_8p", ns_for("MPSC ticket batch 8P/1C", $0), "<=200.00", 200.00)
