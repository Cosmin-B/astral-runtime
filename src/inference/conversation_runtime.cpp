...
inline bool image_desc_valid(const AstralImageDesc* image) {
  return image != nullptr && image->size == sizeof(AstralImageDesc) &&
         image->pixels.data != nullptr && image->pixels.len != 0 && image->width != 0 &&
         image->height != 0;
}

inline bool audio_desc_valid(const AstralAudioDesc* audio) {
  return audio != nullptr && audio->size == sizeof(AstralAudioDesc) &&
         audio->samples.data != nullptr && audio->samples.len != 0 && audio->channels != 0 &&
         audio->sample_rate != 0 && audio->frame_count != 0;
}

inline AstralErr conv_push_text_chunk(Conversation* conv, uint32_t token_start,
                                      uint32_t token_count, uint8_t finalize) {
  if (conv == nullptr) {
...
inline AstralErr conv_push_media_chunk(Conversation* conv, const AstralImageDesc* image,
                                       const AstralAudioDesc* audio, uint8_t finalize) {
  if (conv == nullptr) {
    return ASTRAL_E_INVALID;
  }
  if (conv->prompt_chunk_count >= kMaxPromptChunks) {
  ...
  *out_executor = nullptr;
  ...
  if (ex == nullptr) {
    ...
    if (created == nullptr) {
      ...
      const uint32_t prompt_cap = parse_u32_env("ASTRAL_EXEC_MAX_PROMPT_TOKENS_PER_SLOT_TICK", 8u);
...
AstralErr conv_create_affine(const AstralConvDesc* desc, uint32_t slot_affinity,
                             Conversation** out_conv) {
  ...
  Conversation* conv =
      core::runtime_new<Conversation>(model, allocator_memory, allocator_capacity, *desc);
  if (conv == nullptr) {
  ...
  conv->sample_ids = sample_capacity > 0
                         ? static_cast<uint32_t*>(conv->allocator.alloc(
                               sample_capacity * sizeof(uint32_t), alignof(uint32_t)))
                         : nullptr;
  conv->sample_logits = sample_capacity > 0 ? static_cast<float*>(conv->allocator.alloc(
                                                  sample_capacity * sizeof(float), alignof(float)))
                                            : nullptr;
  ...
  conv->sampler_cfg.seed =
      desc->seed != 0 ? desc->seed : static_cast<uint32_t>(get_ticks() & 0xFFFFFFFFu);
...
void conv_destroy(Conversation* conv) {
  ...
  if (conv->allocator_memory != nullptr) {
    ::astral::core::runtime_session_scratch_release(conv->allocator_memory,
                                                    conv->allocator_capacity);
  }
...
AstralErr conv_feed(Conversation* conv, AstralSpanU8 prompt_chunk, uint8_t finalize) {
  ...
  if (should_tokenize) {
    ...
    const AstralErr err =
        conv->model->backend->ops->tokenize(conv->model->backend_model_ctx, prompt_chunk,
                                            conv->prompt_tokens + conv->prompt_count, space,
                                            /*add_special=*/conv->prompt_count == 0,
                                            /*parse_special=*/false, &n_tokens);
    if (err != ASTRAL_OK) {
      return err;
    }
    if (n_tokens == 0) {
...
AstralErr conv_feed_tokens(Conversation* conv, const int32_t* tokens, uint32_t token_count,
                           uint8_t finalize) {
...
AstralErr conv_feed_image(Conversation* conv, const AstralImageDesc* image, uint8_t finalize) {
  ...
  if (conv->model == nullptr || conv->model->backend == nullptr ||
      conv->model->backend->ops == nullptr ||
...
AstralErr conv_wait(Conversation* conv, uint32_t timeout_ms) {
  ...
  ConvState state = conv->state.load(std::memory_order_acquire);
  if (state == ConvState::Completed)
    return ASTRAL_OK;
  if (state == ConvState::Canceled)
  ...
  const uint32_t check_mask = timeout_ms <= 1u     ? 31u
                              : timeout_ms <= 10u  ? 255u
...
AstralErr conv_penalty_prompt_set_tokens(Conversation* conv, const int32_t* tokens,
                                         uint32_t count) {
  if (conv == nullptr) {
    return ASTRAL_E_INVALID;
  }
...
AstralErr conv_stop_add_utf8(Conversation* conv, AstralSpanU8 utf8) {
  if (conv == nullptr || conv->model == nullptr || conv->model->backend == nullptr ||
      conv->model->backend->ops == nullptr) {
    return ASTRAL_E_INVALID;
  }
  const ConvState st = conv->state.load(std::memory_order_acquire);
  ...
  const AstralErr err = conv->model->backend->ops->tokenize(conv->model->backend_model_ctx, utf8,
                                                            tokens, 32, /*add_special=*/false,
                                                            /*parse_special=*/false, &out_count);
  if (err != ASTRAL_OK) {
    return err;
  }
...
AstralErr conv_grammar_set_gbnf(Conversation* conv, AstralSpanU8 gbnf, AstralSpanU8 root) {
  ...
  const backend::BackendOps* ops =
      (conv->model && conv->model->backend) ? conv->model->backend->ops : nullptr;
  if (ops == nullptr || ops->session_apply_grammar_for_slot == nullptr ||
      ops->session_grammar_set_gbnf_for_slot == nullptr ||
...
int32_t conv_stream_read(Conversation* conv, AstralMutSpanU8 out_buf, uint32_t timeout_ms) {
  ...
  const uint64_t timeout_ticks =
      platform::ticks_from_ns(static_cast<uint64_t>(timeout_ms) * 1000000ULL);
  const uint64_t deadline = start_ticks + timeout_ticks;
  uint32_t spins = 0;
...
int32_t conv_stream_read_meta(Conversation* conv, AstralTokenMeta* out_events, uint32_t capacity,
                              uint32_t timeout_ms) {
  ...
  size_t n = conv->meta_ring.pop_batch(out_events, capacity);
  if (n != 0) {
    platform::cpu_signal_event();
    return static_cast<int32_t>(n);
  }
  ...
  while (get_ticks() < deadline) {
    n = conv->meta_ring.pop_batch(out_events, capacity);
    if (n != 0) {
      platform::cpu_signal_event();
      return static_cast<int32_t>(n);
    }
